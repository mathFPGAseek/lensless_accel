...
-- constants for 64x64 Initialization
constant DELAY8 : integer := 8;
constant FFTSIZE64 : integer := 60; -- FFTSIZE -4 for logic delays?
constant IMAGE64X64 : integer := 4096;
constant IMAGE64X64MINUSONE : integer := 4095;
   ...
   st_mach_controller : process(
          ...
          case ps_controller is
            ...
            when state_wait =>           	
            	...
            	if ( (  fft_rdy_i = '1' ) and
            		   ( state_counter_1_r <= IMAGE64X64MINUSONE)
            		 ) then
            		 ns_controller <= state_read_in_init;
            	else
            ...
            when state_read_in_init =>
            	...
            	if ( state_counter_1_r <= IMAGE64X64) then
            		
            		if ( state_counter_2_r <= FFTSIZE64) then
            		  ns_controller <= state_read_in_init;
            		else
            			ns_controller <= state_wait; 
            		end if;            			
            ...
            when state_read_done =>
            	...
            	if ( state_counter_3_r = DELAY8) then
            		 ns_controller <= state_init;
            	else
            		  ns_controller <= state_read_done;
            	end if;
