...
generic (
  ...
  -- constants
  --
  constant IMAGE64X64    : integer := 4096;
  -- This was bug temporary
  constant COUNT_64       : integer := 63;
  constant COUNT_63       : integer := 62;
  constant COUNT_62       : integer := 61;
  constant COUNT_61       : integer := 60;
  
  constant FFT_IMAGE_SIZE_64  : integer := 61; -- 256 -3 for timing purposes.
  --constant FFT_IMAGE_SIZE  : integer := 254; -- 256 -2 for fix to col_rd
  
  ---
  -- For 256 by 256 Memory
  -- constants
  --
         ...
         case ps_controller is
            ...
            when state_write_in_b =>
            	...
            	if (   (app_rdy_i = '0' ) or 
            		...
            		ns_controller        <= state_init;
            	elsif(state_counter_1_r >= IMAGE64X64 ) then  --  Is amount right
            ...
            when state_wait_for_fft =>
                ...
                elsif(  (state_counter_4_r >= IMAGE64X64 ) and -- complete 1d fft qnd start of col rd for 2d fft
              	  (master_mode_i = "00000") and (stop_fft1d_dbg_i  = '0')           		   	 
              	 ) then 
                	 ns_controller <= state_turnaround;
                	 
                elsif(  (state_counter_14_r >= IMAGE64X64 )  and  -- complete 2d fft and start rd of col for H proc
                ...
                elsif(  (state_counter_14_r >= IMAGE64X64 )  and -- complete H proc
              	      (master_mode_i = "00011")           		   	 
                 ) then 
              	   ns_controller <= state_DEBUG_STOP;
              	   
              	elsif(  (state_counter_14_r >= IMAGE64X64 )  and  -- 
            ...
            when state_wr_1d_fwd_av_row =>
                ...
                elsif(state_counter_3_r >= FFT_IMAGE_SIZE_64  ) then -- complete one FFT write
              	   ns_controller <= state_wait_for_fft;
                --elsif(state_counter_4_r >= IMAGE256X256 ) then -- complete image
              	  --ns_controller <=  state_rd_1d_fwd_av_col;
              	  --  ns_controller <= state_DEBUG_STOP;
            ...
            when state_turnaround =>
            	...
            	if (state_counter_9_r >= COUNT_16 ) then 
            ...
            when state_rd_2d_col  => -- FWD or INV / A or AH Col Rd
              ...
              --if ( (app_rdy_i = '0' )  or (state_counter_7_r >= COUNT_4 )	  ) then
            	...
            	if(state_counter_3_r >= FFT_IMAGE_SIZE_64  ) then -- complete one FFT write
           ...
           when state_wr_col =>
           	   ...
           	   if(state_counter_3_r >= FFT_IMAGE_SIZE_64) then
            ...
            when state_wait_wr_to_rd =>
              ...
              if(  (state_counter_14_r >= IMAGE64X64 )  and 
